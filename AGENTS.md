...
This repository is indexed in GitNexus as DominDev-PT.

Before broad code exploration, feature work, debugging, refactoring, or impact analysis, use the GitNexus MCP server first:
- Read gitnexus://repo/DominDev-PT/context to check repository context and index freshness.
...
- If the index is stale, ask before re-indexing or run gitnexus analyze "D:\ProgramData\DominDev\DominDev-PT" --name DominDev-PT --index-only.
